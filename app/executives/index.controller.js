(function() {
  ...
  angular
    ...
    .controller(
      "Executives.assignedFarmersController",
      assignedFarmersController
    );
  ...
  function Controller(UserService, noty, $uibModal) {
    ...
    function getAllExecutives() {
      UserService.getAllExecutives().then(
        function(data) {
          vm.executives = data;
        },
        function(error) {
  ...
  function addController(UserService, noty, $state) {
    ...
    function addExecutive(form) {
      if (form.$valid) {
        if (vm.isNew) {
          UserService.createExecutive(vm.user).then(
            function(data) {
          ...
          UserService.updateExecutive($state.params.id, vm.user).then(
            function(data) {
              if (data) {
  ...
  function assignedFarmersController(
    UserService,
    noty,
    $uibModalInstance,
    executive
    ...
    function getFarmers(id) {
      UserService.getFarmerByExecutive(id).then(
        function(data) {
          vm.farmers = data;
        },
        function(error) {

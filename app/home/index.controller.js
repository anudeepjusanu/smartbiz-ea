(function() {
  ...
  function Controller(UserService, OrderService, _, $interval, $state) {
    var vm = this;
    vm.farmers = [];
    vm.executives = [];
    vm.date;
    ...
    function GetFarmers() {
      UserService.getAllFarmers().then(
        function(data) {
          vm.farmers = data;
        },
        function(error) {
    ...
    function gotoFarmers() {
      if (
        vm.user &&
        (vm.user.role === "admin" || vm.user.role === "EXECUTIVE")
      ) {
        $state.go("farmers");
      }
    }
    ...
    function getMyOrders(id) {
      OrderService.getOrderByFarmer(id).then(
        function(orders) {
